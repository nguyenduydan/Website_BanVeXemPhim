                ...
                <div class="showtime" id="showtimeContainer">
                    <?php
                        ...
                        if ($resultShowtimes->num_rows > 0) {
                            // Initialize arrays for morning and afternoon/evening showtimes
                            $morningShowtimes = [];
                            $afternoonShowtimes = [];

                            while ($showtime = $resultShowtimes->fetch_assoc()) {
                                $time = strtotime($showtime['giochieu']);
                                if ($time < strtotime('12:00')) {
                                    // Morning showtimes
                                    $morningShowtimes[] = date('H:i', $time);
                                } else {
                                    // Afternoon/Evening showtimes
                                    $afternoonShowtimes[] = date('H:i', $time);
                                }
                            }

                            // Function to display showtimes in a formatted list
                            function displayShowtimes($showtimes, $label)
                            {
                                if (count($showtimes) > 0) {
                                    ...
                                    foreach ($showtimes as $time) {
                                        // Start a new row if count is 5
                                        if ($count % 5 === 0 && $count > 0) {
                                            echo '</ul><ul class="d-flex flex-row flex-wrap justify-content-start">'; // Close and open a new row
                                        }

                                        echo "<li class='time-link mb-3'>$time</li>";
                                        $count++; // Increment the counter
                                    }

                                    echo '</ul>'; // Close the last row
                                }
                            }

                            // Display morning and afternoon/evening showtimes
                            displayShowtimes($morningShowtimes, 'Buổi sáng');
                            displayShowtimes($afternoonShowtimes, 'Buổi chiều và tối');
                        } else {
