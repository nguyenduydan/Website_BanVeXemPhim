...
$inactiveLimit = 1000; // Giới hạn thời gian không hoạt động, ví dụ 10 giây
...
<script>
    ...
    var timeout;
    var userRole = "<?php echo isset($_SESSION['role']) ? $_SESSION['role'] : 'user'; ?>"; // Get user role
    // Kiểm tra nếu phiên đã hết hạn khi trang tải
    window.onload = function() {
        // Check if PHP has set session_expired
        <?php if (isset($_SESSION['session_expired']) && $_SESSION['session_expired'] === true): ?>
            sessionExpired = true; // Set the session expired flag
            if (userRole === 'admin') {
                // Provide a link to redirect
